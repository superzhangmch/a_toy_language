...
static void emit_runtime_impl(LLVMCodeGen *gen) {
    fprintf(gen->out,
        ...
        "  br label %%done\n"
        "done:\n"
        "  %%final = phi i64 [ %%add_result, %%op_add ], [ %%sub_result, %%op_sub ], [ %%mul_result, %%op_mul ], [ %%div_result, %%op_div ], [ %%eq_val, %%op_eq ], [ %%ne_val, %%op_ne ], [ %%lt_val, %%op_lt ], [ %%le_val, %%op_le ], [ %%gt_val, %%op_gt ], [ %%ge_val, %%op_ge ], [ 0, %%default ]\n"
